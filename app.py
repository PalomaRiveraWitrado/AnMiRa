...
# Importamos la vista del administrador que creamos ayer
from admin_dashboard import renderizar_tablero_combos_admin

# =====================================================================
# 1. CONEXIÓN A LA BASE DE DATOS (MySQL Original)
# =====================================================================
def conectar_db():
...
def renderizar_tablero_cobros_admin(id_sucursal, nombre_sucursal):
    ...
    st.caption("Consulta en tiempo real de importes de cobros registrados en Microsip central")
    
    # Tu diccionario exacto y real sincronizado por nombre
    MAPEO_ALMACENES = {
        1: "ALMACEN ORIENTE",       # Arcoiris
        2: "ALMACEN CUATRO CAMINOS",   # 4 Caminos
    ...
    if not nombre_almacen_microsip:
        st.warning("⚠️ Esta sucursal no tiene un almacén enlazado configurado para cobros.")
        return
    ...
    with st.spinner("🔌 Conectando con Firebird y extrayendo cobros..."):
        try:
            from consultas_microsip import obtener_cobros_microsip
            # Enviamos el nombre de texto exacto
    ...
    if df_cobros.empty:
        st.info(f"ℹ️ No se encontraron cobros registrados el día {fecha_consulta.strftime('%d/%m/%Y')} en {nombre_almacen_microsip}.")
        return

    # Procesamos y visualizamos la información de Microsip
    ...
    with col_der:
        ...
        st.dataframe(
            df_mostrar[['FOLIO', 'FORMA_COBRO', 'IMPORTE', 'ALMACEN']], 
...
# =====================================================================
# 👑 VISTA DE ADMINISTRADOR GLOBAL (Con Tablero Comparativo Multisucursal)
# =====================================================================
if st.session_state.get('usuario_rol') == "Admin":
    st.title("👑 Panel de Control Gerencial")

    # 1. Selector de fecha global para todo el panel de administración
    fecha_admin = st.date_input("📅 Selecciona la Fecha a Auditar / Monitorear", date.today(), key="fecha_global_admin")
    st.write("---")

    # 2. DEFINICIÓN DE PESTAÑAS (Añadimos la pestaña de Comparativa Global al inicio)
    tab_comparativa, tab_combos, tab_cobros = st.tabs([
        "📊 Comparativa Global de Sucursales", 
        "🍱 Monitoreo de Combos Individual", 
        "💳 Auditoría de Cobros en Vivo"
    ])

    # Mapeo maestro de sucursales y almacenes para consultas
    SUCURSALES_MAP = {
        1: "Arcoiris",
        2: "4 Caminos",
        3: "Triana",
    ...
    # =====================================================================
    # PESTAÑA 1: COMPARATIVA GLOBAL DE SUCURSALES (Tus datos unificados)
    # =====================================================================
    with tab_comparativa:
        st.markdown(f"### 🏁 Rendimiento Cruzado del Día: **{fecha_admin.strftime('%d/%m/%Y')}**")
        st.caption("Resumen consolidado extrayendo datos en vivo de Microsip y los reportes guardados en MySQL.")
        
        # Botón para forzar la actualización de todas las sucursales a la vez
        if st.button("🔄 Sincronizar y Recargar Todas las Sucursales", key="btn_recarga_global"):
            st.rerun()

        # Creamos una lista para ir guardando los renglones de nuestra gran tabla comparativa
        datos_comparativos = []

        with st.spinner("📊 Consolidando información de todos los servidores... Por favor espere..."):
            for id_suc, nom_suc in SUCURSALES_MAP.items():
                nombre_almacen_ms = MAPEO_ALMACENES_NOMBRES.get(id_suc, "")
                
                # --- A) DATOS DE MICROSIP (Cobros y Combos) ---
                total_cobrado_ms = 0.0
                total_combos_ms = 0
                
                if nombre_almacen_ms:
                    try:
                        # Consulta de combos vendidos a $0
                        from consultas_microsip import obtener_ventas_cero_microsip
                        df_c_ms = obtener_ventas_cero_microsip(fecha_admin, nombre_almacen_ms)
                        if not df_c_ms.empty and "UNIDADES" in df_c_ms.columns:
                            total_combos_ms = int(df_c_ms["UNIDADES"].sum())
                            
                        # Consulta de cobros totales
                        from consultas_microsip import obtener_cobros_microsip
                        df_p_ms = obtener_cobros_microsip(fecha_admin, nombre_almacen_ms)
                        if not df_p_ms.empty:
                            df_p_ms.columns = [c.upper() for c in df_p_ms.columns]
                            if 'IMPORTE' in df_p_ms.columns:
                                total_cobrado_ms = float(df_p_ms['IMPORTE'].sum())
                    except Exception as e:
                        pass # Si una sucursal falla temporalmente en conexión, se queda en 0 y no tumba el panel

                # --- B) DATOS DE MYSQL LOCAL (Reportes de Caja guardados) ---
                estado_reporte = "❌ Sin Guardar"
                efectivo_contado_db = 0.0
                diferencia_caja = 0.0
                
                try:
                    conn_comp = conectar_db()
                    cursor_comp = conn_comp.cursor(dictionary=True)
                    
                    # Verificamos si ya guardaron reporte
                    cursor_comp.execute("SELECT id_reporte, ventas_efectivo_sistema, fondo_inicial, fondo_siguiente_dia FROM reportes_caja WHERE id_sucursal = %s AND fecha = %s", (id_suc, fecha_admin))
                    rep_local = cursor_comp.fetchone()
                    
                    if rep_local:
                        estado_reporte = "✅ Guardado"
                        id_r = rep_local['id_reporte']
                        v_efe = float(rep_local['ventas_efectivo_sistema'])
                        f_ini = float(rep_local['fondo_inicial'])
                        
                        # Sumamos gastos autorizados
                        cursor_comp.execute("SELECT SUM(monto) as total_g FROM gastos_autorizados WHERE id_reporte = %s", (id_r,))
                        g_res = cursor_comp.fetchone()
                        g_tot = float(g_res['total_g']) if g_res and g_res['total_g'] else 0.0
                        
                        efectivo_esperado = (v_efe + f_ini) - g_tot
                        
                        # Traemos el conteo físico real de los billetes
                        cursor_comp.execute("SELECT total_efectivo_fisico FROM desglose_efectivo WHERE id_reporte = %s", (id_r,))
                        efe_res = cursor_comp.fetchone()
                        if efe_res:
                            efectivo_contado_db = float(efe_res['total_efectivo_fisico'])
                            diferencia_caja = efectivo_contado_db - efectivo_esperado
                ...
                # Guardamos los resultados consolidados de esta sucursal
                datos_comparativos.append({
                    "Sucursal": nom_suc,
                    "Estatus Caja Local": estado_reporte,
                    "Cobrado en Microsip": total_cobrado_ms,
                    "Efectivo Contado (Físico)": efectivo_contado_db,
                    "Descuadre / Diferencia": diferencia_caja,
                    "Componentes Combo ($0)": total_combos_ms
                })

        # 3. CONVERSIÓN A DATAFRAME Y RENDERIZADO DE LA COMPARATIVA
        df_comparativo = pd.DataFrame(datos_comparativos)
        
        # Encontramos de forma dinámica la sucursal líder en ventas hoy
        if df_comparativo["Cobrado en Microsip"].sum() > 0:
            sucursal_lider = df_comparativo.loc[df_comparativo["Cobrado en Microsip"].idxmax()]["Sucursal"]
            venta_maxima = df_comparativo["Cobrado en Microsip"].max()
            st.success(f"🏆 **Sucursal Líder del Día:** ¡**{sucursal_lider}** lidera la empresa con **${venta_maxima:,.2f}** registrados en Microsip!")
        else:
            st.info("ℹ️ Aún no se registran movimientos de cobro en Microsip para esta fecha.")

        # Formateamos las columnas numéricas para que se vean profesionales con signo de pesos
        df_visualizacion = df_comparativo.copy()
        df_visualizacion["Cobrado en Microsip"] = df_visualizacion["Cobrado en Microsip"].map(lambda x: f"${x:,.2f}")
        df_visualizacion["Efectivo Contado (Físico)"] = df_visualizacion["Efectivo Contado (Físico)"].map(lambda x: f"${x:,.2f}")
        df_visualizacion["Descuadre / Diferencia"] = df_visualizacion["Descuadre / Diferencia"].map(lambda x: f"${x:,.2f}" if x == 0 else (f"🚨 ${x:,.2f} Faltante" if x < 0 else f"✅ +${x:,.2f} Sobrante"))
        df_visualizacion["Componentes Combo ($0)"] = df_visualizacion["Componentes Combo ($0)"].map(lambda x: f"{x} pzas")

        # Pintamos la mega tabla de comparación empresarial
        st.dataframe(df_visualizacion, use_container_width=True, hide_index=True)


    # =====================================================================
    # PESTAÑA 2: MONITOREO DE COMBOS INDIVIDUAL (Tu código original corregido)
    # =====================================================================
    with tab_combos:
        suc_sel_combos = st.selectbox("🔍 Selecciona sucursal para desglose de combos:", options=list(SUCURSALES_MAP.keys()), format_func=lambda x: SUCURSALES_MAP[x], key="sb_combos_ind")
        nombre_almacen_ms = MAPEO_ALMACENES_NOMBRES.get(suc_sel_combos, None)
        
        if nombre_almacen_ms:
            with st.spinner("🔌 Conectando con Microsip..."):
                try:
                    from consultas_microsip import obtener_ventas_cero_microsip
                    df_combos = obtener_ventas_cero_microsip(fecha_admin, nombre_almacen_ms)
                    if not df_combos.empty:
                        total_unidades = df_combos["UNIDADES"].sum()
                        st.metric(label="🥡 Total Componentes Servidos (Cocina)", value=f"{int(total_unidades)} pzas")
                        st.dataframe(df_combos, use_container_width=True, hide_index=True)
                    else:
                        st.info(f"ℹ️ No se detectan componentes de combos vendidos a $0 en Microsip el {fecha_admin.strftime('%d/%m/%Y')} en {nombre_almacen_ms}.")
                except Exception as e:
                    st.error(f"Error en pestaña combos: {e}")


    # =====================================================================
    # PESTAÑA 3: AUDITORÍA DE COBROS EN VIVO (Tu código original corregido)
    # =====================================================================
    with tab_cobros:
        suc_sel_cobros = st.selectbox("🔍 Selecciona sucursal para desglose de cobros:", options=list(SUCURSALES_MAP.keys()), format_func=lambda x: SUCURSALES_MAP[x], key="sb_cobros_ind")
        nombre_almacen_ms = MAPEO_ALMACENES_NOMBRES.get(suc_sel_cobros, None)
        
        if nombre_almacen_ms:
            with st.spinner("🔌 Conectando con Microsip..."):
                try:
                    from consultas_microsip import obtener_cobros_microsip
                    df_cobros = obtener_cobros_microsip(fecha_admin, nombre_almacen_ms)
                    if not df_cobros.empty:
                        ...
                        resumen_pagos = df_cobros.groupby('FORMA_COBRO')['IMPORTE'].sum().reset_index()
                        cols_m = st.columns(len(resumen_pagos))
                        for idx, r_pago in resumen_pagos.iterrows():
                            with cols_m[idx]:
                                st.metric(label=f"💰 {r_pago['FORMA_COBRO']}", value=f"${r_pago['IMPORTE']:,.2f}")
                        st.write("---")
                        st.dataframe(df_cobros[['FOLIO', 'FORMA_COBRO', 'IMPORTE']], use_container_width=True, hide_index=True)
                    else:
                        st.info(f"ℹ️ No se encontraron cobros registrados en Microsip el {fecha_admin.strftime('%d/%m/%Y')} en {nombre_almacen_ms}.")
                except Exception as e:
                    st.error(f"Error en pestaña cobros: {e}")


    # =====================================================================
    # SECCIÓN INFERIOR: SECTOR DE IMPRESIÓN INDIVIDUAL EN BASE A MYSQL
    # =====================================================================
    st.write("---")
    st.markdown("### 📋 Sección de Impresión Individual de Reportes Locales")
    
    # Permitimos al admin elegir cuál sucursal quiere descargar en PDF de la fecha seleccionada
    suc_imprimir = st.selectbox("🖨️ Selecciona la sucursal para generar su reporte PDF de cierre:", options=list(SUCURSALES_MAP.keys()), format_func=lambda x: SUCURSALES_MAP[x], key="sb_imprimir_admin")
    ...
    try:
        ...
        cursor_admin.execute(query_load_rep, (suc_imprimir, fecha_admin))
    ...
    if not reporte_existente_admin:
        st.warning(f"⚠️ No se encontró ningún reporte registrado en la base de datos local para la sucursal {SUCURSALES_MAP[suc_imprimir]} el día {fecha_admin.strftime('%d/%m/%Y')}. El PDF se descargará con la estructura base.")
        
        paquete_datos_pdf = {
            "sucursal": SUCURSALES_MAP[suc_imprimir], "fecha": str(fecha_admin),
            "fondo_inicial": 0.0, "fondo_siguiente": 0.0, "cocineros": "Sin turno guardado",
            "v_efectivo": 0.0, "v_tarjetas": 0.0, "v_apps": 0.0, "v_total": 0.0,
            "efectivo_esperado": 0.0, "efectivo_contado": 0.0, "gastos": [],
            "refrescos_vendidos": 0, "combos_donacion": 0, "platillos_resumen": []
        }
        paquete_efectivo_real = {"b500":0,"b200":0,"b100":0,"b50":0,"b20":0,"m10":0,"m5":0,"m2":0,"m1":0,"m05":0}
        historial_pdf_actualizado = []
    else:
        st.success(f"✅ Reporte de caja local encontrado para {SUCURSALES_MAP[suc_imprimir]} (ID Reporte: {reporte_existente_admin['id_reporte']})")
        
        # ... [Toda tu lógica original de recolección de paquetes de datos de comida, gastos e inventario para armar el PDF se mantiene intacta aquí abajo] ...
        historial_pdf_actualizado = []
        ...
        try:
            ...
            if enc_bd: val_cocineros_admin = enc_bd['cocineros']
                
            cursor_pdf.close()
            conn_pdf.close()
        except Exception as e_fetch: pass
        ...
        paquete_datos_pdf = {
            "sucursal": SUCURSALES_MAP[suc_imprimir], "fecha": str(reporte_existente_admin['fecha']),
            "fondo_inicial": f_ini, "fondo_siguiente": float(reporte_existente_admin['fondo_siguiente_dia']), "cocineros": val_cocineros_admin,
            "v_efectivo": v_efe, "v_tarjetas": v_tar, "v_apps": v_app, "v_total": v_tot,
        ...
        paquete_efectivo_real = {"b500": int(efectivo_existente_admin['b500']) if efectivo_existente_admin else 0, "b200": int(efectivo_existente_admin['b200']) if efectivo_existente_admin else 0, "b100": int(efectivo_existente_admin['b100']) if efectivo_existente_admin else 0, "b50": int(efectivo_existente_admin['b50']) if efectivo_existente_admin else 0, "b20": int(efectivo_existente_admin['b20']) if efectivo_existente_admin else 0, "m10": int(efectivo_existente_admin['m10']) if efectivo_existente_admin else 0, "m5": int(efectivo_existente_admin['m5']) if efectivo_existente_admin else 0, "m2": int(efectivo_existente_admin['m2']) if efectivo_existente_admin else 0, "m1": int(efectivo_existente_admin['m1']) if efectivo_existente_admin else 0, "m05": int(float(efectivo_existente_admin['m05'])) if efectivo_existente_admin else 0}

    # Despliegue permanente del botón de descarga PDF de la sucursal elegida
    try:
        st.download_button(
            label=f"📥 Generar y Descargar PDF de Sucursal: {SUCURSALES_MAP[suc_imprimir]}",
    ...
    # 🛑 FRENO ABSOLUTO: El Administrador termina aquí y jamás ve el formulario de captura física del cajero
    st.stop()



# =====================================================================
# 👤 VISTA EXCLUSIVA PARA LOS CAJEROS Y SUCURSALES (AQUÍ EMPIEZA TU FORMULARIO)
# =====================================================================
else:
    st.title("🥡 Control de Reporte de Caja")
    st.subheader("Complete los campos según el formato físico del día.")
